...
import { Plus, Trash2, Warehouse, X, Building2, MapPin, ChevronDown } from 'lucide-react';
...
export default function WarehouseList() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      // Depoları ve verilecek olan kurumu çekiyoruz (Artık şubeleri çekiyoruz)
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.branchId) {
      toast.error('Lütfen bir firma/şube seçiniz.');
      return;
    }
    ...
    try {
      ...
      const payload = {
        name: formData.name,
        branchId: formData.branchId, // DTO string olarak bekliyor, formData objemizden direkt alıyoruz
        city: formData.city,
      ...
      setFormData({ name: '', branchId: '', city: '', district: '', detailedAddress: '' });
      fetchData();
    } catch (error) {
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Depo Adı</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Bağlı Firma/Şube</th>
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {item.branch?.company?.name || 'Firma Yok'} - ({item.branch?.name || 'Şube Yok'})
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                ...
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-gray-500">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p>Henüz hiçbir depo eklenmemiş.</p>
                    </div>
                  </td>
                </tr>
              )}
            ...
            <div className="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-gray-100">
              <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        >
                          <option value="" disabled>Firma/Şube Seçiniz</option>
                          {branches.map((branch) => (
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Örn: Akdeniz Dağıtım Merkezi"
                      />
                    </div>
                  </div>

                  {/* Lokasyon: İl ve İlçe (Grid) */}
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                  ...
                  {/* Yalnızca Submit / İptal butonları */}
