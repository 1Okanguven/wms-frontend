...
import { Plus, Trash2, Building, X, Building2, ChevronDown } from 'lucide-react';
...
export default function BranchList() {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error('Veriler yüklenirken hata oluştu. (Branch)');
    } finally {
      setLoading(false);
    }
  };
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Bağlı Firma</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {loading ? (
                ...
                <tr><td colSpan="4" className="px-6 py-12 text-center text-gray-500">Henüz hiçbir şube eklenmemiş.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-500 bg-opacity-75">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden border border-gray-100 animate-fade-in">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
              <h3 className="text-lg font-semibold flex items-center gap-2"><Building className="h-5 w-5 text-blue-600" /> Yeni Şube Ekle</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Bağlı Firma <span className="text-red-500">*</span></label>
                  <div className="relative">
                    ...
                    <select name="companyId" required value={formData.companyId} onChange={handleInputChange} className="block w-full rounded-lg border-gray-300 pl-10 py-2.5 text-gray-900 shadow-sm focus:ring-blue-500 border appearance-none">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Şube Adı <span className="text-red-500">*</span></label>
                  <input type="text" name="name" required value={formData.name} onChange={handleInputChange} className="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 p-2.5 border" placeholder="Örn: Kadıköy Şubesi" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">İl</label>
                  <input type="text" name="city" value={formData.city} onChange={handleInputChange} className="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 p-2.5 border" placeholder="Örn: İstanbul" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">İlçe</label>
                  <input type="text" name="district" value={formData.district} onChange={handleInputChange} className="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 p-2.5 border" placeholder="Örn: Kadıköy" />
                </div>
              </div>
              <div className="mt-4">
                <label className="block text-sm font-medium text-gray-700 mb-1">Açık Adres</label>
                <textarea name="detailedAddress" rows="2" value={formData.detailedAddress} onChange={handleInputChange} className="block w-full rounded-lg border-gray-300 shadow-sm focus:ring-blue-500 p-2.5 border" placeholder="Adres detayları..."></textarea>
              </div>
